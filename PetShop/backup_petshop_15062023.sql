...
-- Copiando dados para a tabela petshop.logcliente: ~9 rows (aproximadamente)
...
-- Copiando dados para a tabela petshop.logpet: ~9 rows (aproximadamente)
...
-- Copiando estrutura para procedure petshop.proc_addEstoqueProduto
DROP PROCEDURE IF EXISTS `proc_addEstoqueProduto`;
DELIMITER //
...
CREATE TABLE `v_pets` (
	...
	`Raca` VARCHAR(100) NOT NULL COLLATE 'utf8_general_ci',
	`Porte` VARCHAR(50) NOT NULL COLLATE 'utf8_general_ci',
...
CREATE TRIGGER `tri_LogAlteraCliente` AFTER UPDATE ON `cliente` FOR EACH ROW BEGIN 
	...
	IF(old.emailCliente IS NULL and NEW.emailCliente IS NOT NULL) then
		SET @mensagem = CONCAT("E-mail Alterado: NULL para ",NEW.emailCliente);
		INSERT INTO logcliente VALUES (NULL,"Update","cliente",NEW.codCliente,@mensagem,USER(),NOW());
	END if;
	...
	IF(old.enderecoCliente IS NULL and NEW.enderecoCliente IS NOT NULL) then
		SET @mensagem = CONCAT("Endereço Alterado: NULL para ",NEW.enderecoCliente);
		INSERT INTO logcliente VALUES (NULL,"Update","cliente",NEW.codCliente,@mensagem,USER(),NOW());
	END if;
	...
	IF(old.bairroCliente IS NULL and NEW.bairroCliente IS NOT NULL) then
		SET @mensagem = CONCAT("Bairro Alterado: NULL para ",NEW.bairroCliente);
		INSERT INTO logcliente VALUES (NULL,"Update","cliente",NEW.codCliente,@mensagem,USER(),NOW());
	END if;
	...
	IF(old.dataNascimento IS NULL and NEW.dataNascimento IS NOT NULL) then
		SET @mensagem = CONCAT("Nascimento Alterado: NULL para ",NEW.dataNascimento);
		INSERT INTO logcliente VALUES (NULL,"Update","cliente",NEW.codCliente,@mensagem,USER(),NOW());
	END if;
...
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tri_LogAlteraFuncionario` AFTER UPDATE ON `funcionario` FOR EACH ROW BEGIN 
	...
	IF(OLD.dataAdmissao IS NULL and NEW.dataAdmissao IS NOT NULL) then
		SET @mensagem = CONCAT("Admissao Alterada: NULL para ",NEW.dataAdmissao);
		INSERT INTO logfuncionario VALUES (NULL,"Update","funcionario",NEW.codFuncionario,@mensagem,USER(),NOW());
	END if;
	...
	IF(OLD.dataDemissao IS NULL and NEW.dataDemissao IS NOT NULL) then
		SET @mensagem = CONCAT("Demissao Alterada: NULL para ",NEW.dataDemissao);
		INSERT INTO logfuncionario VALUES (NULL,"Update","funcionario",NEW.codFuncionario,@mensagem,USER(),NOW());
	END if;
